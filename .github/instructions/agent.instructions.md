...
- calculate_anomaly(current, baseline) → deviation (absolute + %)
- check_significance(anomaly, std_dev) → bool, using threshold rule:
  significant if anomaly > 1.5 standard deviations from historical mean
...
3. calculate_anomaly(current, baseline)
